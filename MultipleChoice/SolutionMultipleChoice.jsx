...
function SolutionMultipleChoice({model,type}) {
  ...
  return (
    <div className={`${styles.solutionPage} ${styles.solutionPage2}`} style={{
      fontWeight:'initial',fontSize:'initial',lineHeight:'initial'
    }}>
      <div className={styles.solutionText}><u>Solution: </u></div>
      {model?.map((item, i) => (
        (item.correct||String(type).trim()==("Fill in the blanks ").trim())&&<>
