...
const validationForKeying=(inputRef)=>{
  ...
  for(let i=0;i<n;i++)
  {
    ...
    {
      if(arr[i][j].isMissed==="true")
      if(String(arr[i][j]?.dropVal).trim()?.toLowerCase()!=String(arr[i][j]?.value).trim()?.toLowerCase())
      return 1
    }
  }
  return 2
}
export default function LongDivsion({ state, totalRows, totalCols,meter }) {
  ...
  const handleSubmitAnswer = () => {
    ...
    setQuestionWithAnswer({...state,[student_answer]:inputRef.current?.filter(item=>item.show)[0]?.value||""})
