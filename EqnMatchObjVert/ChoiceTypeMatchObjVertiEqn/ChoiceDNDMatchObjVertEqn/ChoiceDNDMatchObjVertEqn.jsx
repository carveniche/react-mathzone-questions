    ...
    <>
      {dropState?.map((items, index) => (
        <div key={index}>
          {items?.map((item, i) =>
            item.isMissed !== "true" ? (
             <div className={styles.MatchObjectVerticalDragDropFlexBox3} style={{fontSize:16,fontWeight:"bold"}}>
                <div className="fontSize" style={{color:"black",fontSize:16,fontWeight:"bold",gap:"1rem"}}>{parse(item?.numvalue,optionSelectStaticMathField)}</div>
              <div className="fontSize" style={{color:"black",fontSize:16,fontWeight:"bold",gap:"1rem"}}>{parse(item?.imgvalue,optionSelectStaticMathField)}</div>
             </div>
            ) : (
              ...
              >
                {(item.show||isStudentAnswerResponse )&& (
                  <Draggable onStop={(e) => handleStop2(e, index, i)} disabled={hasAnswerSubmitted||isStudentAnswerResponse} onStart={handleDragStart} onDrag={handleDrag}>
                    <div>{parse(isStudentAnswerResponse?item[student_answer]:item.dropVal,optionSelectStaticMathField)}</div>
                  </Draggable>
                )}
              </div>
               </div>
      ...
      <div className={styles.MatchObjectVerticalDragDropBoxFlexBox2}>
        {dragState?.map((items, i) => (
          <div id={`${i}`} className={`draggablehfu ${styles.MatchObjectVerticalDragDropBox}`}   style={
            ...
            {items.show && (
              <Draggable onStop={(e) => handleStop1(e, i)} disabled={hasAnswerSubmitted||isStudentAnswerResponse} onStart={handleDragStart} onDrag={handleDrag}>
