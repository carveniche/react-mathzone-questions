...
export default function SelectChoice({choices,inputRef,answerHasSelected,content,totalRows}){
   ...
   {row?.map((items,index)=><FlexBox key={index}>
    {items?.map((item,i)=>item.isMissed==="false"?<div key={i}>{typeof item?.value==="string"?HtmlParser(item.value):item.value}</div>:<div value={item.value} key={i}>
            <Input disabled={true}/>
            </div>)}
        </FlexBox>)}
